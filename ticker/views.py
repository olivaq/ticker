...
def nodeedit(request, node_id=1):
    ...
    return render_to_response('ticker/node_edit.html', {'node':node,'form':form},context_instance=RequestContext(request))

def dump_run(request, dump_id):
    dump = models.LinkDump.objects.get(id = dump_id)
    data = pickle.loads(dump.dump.encode('ascii'))
    
    res = runcode.apply_async((dump.link.dst.id,), kwargs={'locals_dict':data})
    return HttpResponse("lol %s"%res.id)
